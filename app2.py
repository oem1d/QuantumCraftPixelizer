...
with col_about:
    st.button("About", key="nav_about", on_click=set_page, args=("about",))
...
if page == "create":
    ...
    if uploaded_file is not None:
        ...
        if final_pixel_art is not None:
            ...
            with col_a:
                cell_size = st.slider("Preview cell size", 8, 32, 18)
            ...
            if "shared_patterns" not in st.session_state:
                st.session_state.shared_patterns = []
            ...
            if st.button("Save to Gallery"):
                st.session_state.shared_patterns.append({
                    "title": pattern_title,
                    "image": craft_image
                })
    ...
    if len(st.session_state.shared_patterns) == 0:
        st.info("No saved patterns yet.")
    else:
        cols = st.columns(3)

        for i, item in enumerate(st.session_state.shared_patterns):
            with cols[i % 3]:
    ...
    st.write(
        ...
        clustering and palette mapping to generate craft patterns.
        """
    )